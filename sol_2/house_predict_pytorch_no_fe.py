...
def train_model(model, train_loader, val_loader, epochs=100, learning_rate=0.001,
                device='cpu', early_stopping=True, patience=15):
    """
    Args:
        model: 神经网络模型
    ...
    Returns:
        training_history: 训练历史记录（损失、准确率和内存使用）
    """
...
def plot_training_history(history):
    ...
    # 绘制内存使用曲线
    if 'memory_mb' in history and history['memory_mb']:
        axes[2].plot(history['memory_mb'], label='Memory Usage', marker='o', color='green')
        axes[2].set_xlabel('Epoch')
        axes[2].set_ylabel('Memory Usage (MB)')
        axes[2].set_title('Memory Usage During Training')
        axes[2].legend()
        axes[2].grid(True)
...
if __name__ == '__main__':
    ...
    # 打印内存统计
    if 'memory_mb' in history and history['memory_mb']:
        memory_values = history['memory_mb']
